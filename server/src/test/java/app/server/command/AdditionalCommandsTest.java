...
public class AdditionalCommandsTest extends BaseDAOTest {
  ...
  @Test
  public void testFetchAuctionDetailCommand_withViewingAuction() {
    // Test FetchAuctionDetail voi session dang xem auction (setViewingAuctionId)
    FetchAuctionDetailCommand cmd = new FetchAuctionDetailCommand(queryService, autoBidService);
    PacketReq req =
        PacketReq.of(RequestType.FETCH_AUCTION_DETAIL, new AuctionDetailRequest(auction.getId()));

    fakeClientHandler.setFakeUser(bidder);
    // Gia lap user dang xem auction khac
    fakeClientHandler.getSession().setViewingAuctionId(auction.getId());
    cmd.execute(fakeClientHandler, req);

    PacketRes res = fakeClientHandler.getSentPacket();
    assertNotNull(res);
    // Session viewingAuctionId phai duoc cap nhat
    assertEquals(auction.getId(), fakeClientHandler.getSession().getViewingAuctionId());
  }

  @Test
  public void testCreateAuctionCommand_missingPayload() {
    CreateAuctionCommand cmd = new CreateAuctionCommand(auctionService, queryService);
    // Gui request khong co payload
    PacketReq req = PacketReq.of(RequestType.CREATE_AUCTION, null);
  ...
  @Test
  public void testFetchSellerItemsCommand_missingPayload() {
    FetchSellerItemsCommand cmd = new FetchSellerItemsCommand(itemService);
    PacketReq req = PacketReq.of(RequestType.FETCH_SELLER_ITEMS, null);
  ...
  @Test
  public void testCancelAuctionCommand_missingPayload() {
    CancelAuctionCommand cmd = new CancelAuctionCommand(auctionService, queryService, userService);
    PacketReq req = PacketReq.of(RequestType.CANCEL_AUCTION, null);
  ...
  @Test
  public void testPlaceBidCommand_missingPayload() {
    PlaceBidCommand cmd = new PlaceBidCommand(bidService, userService, queryService);
    PacketReq req = PacketReq.of(RequestType.PLACE_BID, null);
  ...
  @Test
  public void testDepositCommand_missingPayload() {
    DepositCommand cmd = new DepositCommand(userService);
    PacketReq req = PacketReq.of(RequestType.DEPOSIT, null);
  ...
  @Test
  public void testBanUserCommand_missingPayload() {
    BanUserCommand cmd = new BanUserCommand(userService);
    PacketReq req = PacketReq.of(RequestType.BAN_USER, null);
  ...
  @Test
  public void testUpdateAuctionCommand_missingPayload() {
    UpdateAuctionCommand cmd = new UpdateAuctionCommand(auctionService, queryService);
    PacketReq req = PacketReq.of(RequestType.UPDATE_AUCTION, null);
  ...
  @Test
  public void testSetAutoBidCommand_missingPayload() {
    SetAutoBidCommand cmd = new SetAutoBidCommand(autoBidService, queryService);
    PacketReq req = PacketReq.of(RequestType.SET_AUTO_BID, null);
  ...
  @Test
  public void testDisableAutoBidCommand_missingPayload() {
    DisableAutoBidCommand cmd = new DisableAutoBidCommand(autoBidService);
    PacketReq req = PacketReq.of(RequestType.DISABLE_AUTO_BID, null);
  ...
  @Test
  public void testFetchItemImageCommand_itemHasNoImage() {
    FetchItemImageCommand cmd = new FetchItemImageCommand(imageStorageService, itemService);
    // Item chua co image URL
    PacketReq req =
        PacketReq.of(RequestType.FETCH_ITEM_IMAGE, new FetchItemImageRequest(item.getId()));
    ...
    PacketRes res = fakeClientHandler.getSentPacket();
    assertNotNull(res);
    // Nen tra loi vi item chua co image
    assertFalse(res.isSuccess());
  }

  @Test
  public void testUploadImageCommand_missingPayload() {
    UploadImageCommand cmd = new UploadImageCommand(itemService, imageStorageService, queryService);
    PacketReq req = PacketReq.of(RequestType.UPLOAD_IMAGE, null);
  ...
  @Test
  public void testUploadAvatarCommand_withoutLoginUser() {
    UploadAvatarCommand cmd = new UploadAvatarCommand(userService, imageStorageService);
    String base64 = java.util.Base64.getEncoder().encodeToString("data".getBytes());
    PacketReq req =
        PacketReq.of(RequestType.UPLOAD_AVATAR, new UploadAvatarRequest(base64, "avatar.png"));

    // Khong setFakeUser -> user == null
    cmd.execute(fakeClientHandler, req);
  ...
  @Test
  public void testUploadAvatarCommand_nullRequestData() {
    UploadAvatarCommand cmd = new UploadAvatarCommand(userService, imageStorageService);
    // Gui request voi null payload
    PacketReq req = PacketReq.of(RequestType.UPLOAD_AVATAR, null);
  ...
  @Test
  public void testFetchAvatarCommand_nullAvatarUrl() {
    FetchAvatarCommand cmd = new FetchAvatarCommand(imageStorageService);
    // Gui request voi avatarUrl null
    PacketReq req =
        PacketReq.of(RequestType.FETCH_AVATAR, new FetchAvatarRequest(bidder.getId(), null));
  ...
  @Test
  public void testFetchAvatarCommand_blankAvatarUrl() {
    FetchAvatarCommand cmd = new FetchAvatarCommand(imageStorageService);
    // Gui request voi avatarUrl blank
    PacketReq req =
        PacketReq.of(RequestType.FETCH_AVATAR, new FetchAvatarRequest(bidder.getId(), "  "));
  ...
  @Test
  public void testFetchAvatarCommand_nonExistentPath() {
    FetchAvatarCommand cmd = new FetchAvatarCommand(imageStorageService);
    // Gui request voi path khong ton tai -> IOException hoac IllegalArgumentException
    PacketReq req =
        PacketReq.of(
            RequestType.FETCH_AVATAR,
            new FetchAvatarRequest(bidder.getId(), "server_data/images/non_existent_avatar.png"));

    cmd.execute(fakeClientHandler, req);

    PacketRes res = fakeClientHandler.getSentPacket();
    assertNotNull(res);
    // File khong ton tai nen tra loi loi
    assertFalse(res.isSuccess());
  }

  @Test
  public void testSettleWalletCommand_withWinnerPaidPath() {
    SettleWalletCommand cmd = new SettleWalletCommand(auctionService, queryService, userService);

    // Setup phien FINISHED voi winner va frozen funds de paidAmount > 0
    auction.setStatus(AuctionStatus.FINISHED);
    auction.updateHighestBid(1500L, bidder.getId());
    auctionDAO.update(auction);

    // Nap tien va freeze cho bidder
    bidder = userDAO.findById(bidder.getId()).orElseThrow();
    bidder.getWallet().deposit(new BigDecimal("5000"));
    userDAO.update(bidder);
    bidder = userDAO.findById(bidder.getId()).orElseThrow();
    bidder.getWallet().setFrozenAmount(String.valueOf(auction.getId()), new BigDecimal("1500"));
    userDAO.update(bidder);
    ...
    PacketRes res = fakeClientHandler.getSentPacket();
    assertNotNull(res);
    // SettleWalletCommand nen thanh cong (paidAmount > 0 sau completeAuction)
    assertTrue(res.isSuccess());
  }

  @Test
  public void testCancelAuctionCommand_runningAuctionWithBidder() {
    CancelAuctionCommand cmd = new CancelAuctionCommand(auctionService, queryService, userService);

    // Chuyen phien sang RUNNING
    auction.start();
    auctionDAO.update(auction);

    // Nap tien va bidder dat gia (tao frozen funds)
    bidder = userDAO.findById(bidder.getId()).orElseThrow();
    bidder.getWallet().deposit(new BigDecimal("5000"));
    userDAO.update(bidder);
    bidder = userDAO.findById(bidder.getId()).orElseThrow();
    ...
    // Cap nhat highestBid trong auction
    auction = auctionDAO.findById(auction.getId()).orElseThrow();
    ...
    PacketRes res = fakeClientHandler.getSentPacket();
    assertNotNull(res);
    // Admin huy phien dang chay co bidder -> releasedUserIds khong rong -> sendWalletUpdates duoc
    // goi
    assertTrue(res.isSuccess());
    assertEquals(ResponseType.CANCEL_AUCTION_RESULT, res.getType());
  }
  ...
  @Test
  public void testSettleWalletCommand_invalidAuctionId() {
    SettleWalletCommand cmd = new SettleWalletCommand(auctionService, queryService, userService);
    PacketReq req = PacketReq.of(RequestType.SETTLE_WALLET, new SettleWalletRequest(0));
  ...
  @Test
  public void testFetchAuctionDetailCommand_notModified() {
    FetchAuctionDetailCommand cmd = new FetchAuctionDetailCommand(queryService, autoBidService);
    Auction latest = queryService.getAuction(auction.getId());
    PacketReq req =
        PacketReq.of(
            RequestType.FETCH_AUCTION_DETAIL,
            new AuctionDetailRequest(latest.getId(), latest.getVersion()));
    ...
    System.out.println(
        "DEBUG latest_version: "
            + latest.getVersion()
            + ", req_knownVersion: "
            + req.getData(AuctionDetailRequest.class).knownVersion()
            + ", db_version: "
            + queryService.getAuction(auction.getId()).getVersion());
    cmd.execute(fakeClientHandler, req);

    PacketRes res = fakeClientHandler.getSentPacket(ResponseType.AUCTION_RESULT);
    assertNotNull(res);
    System.out.println("DEBUG PACKET JSON: " + res.toString());
    assertTrue(res.isSuccess());
    AuctionDetailResponse data = res.getData(AuctionDetailResponse.class);
    assertNotNull(data);
    assertTrue(data.notModified());
  }

  @Test
  public void testFetchAuctionDetailCommand_withAutoBid() {
    ...
    PacketReq req =
        PacketReq.of(
            RequestType.FETCH_AUCTION_DETAIL, new AuctionDetailRequest(auction.getId(), -1));
  ...
  @Test
  public void testFetchAuctionHistoryCommand_sinceVersion() {
    FetchAuctionHistoryCommand cmd = new FetchAuctionHistoryCommand(queryService);
    PacketReq req = PacketReq.of(RequestType.FETCH_AUCTION_HISTORY, new AuctionHistoryRequest(1));
  ...
  @Test
  public void testPasswordUtilsConstructor() {
    assertNotNull(new app.server.utils.PasswordUtils());
  }

  @Test
  public void testUnwatchAuctionCommand_coverage() {
    UnwatchAuctionCommand cmd = new UnwatchAuctionCommand();
    assertEquals(ResponseType.ERROR, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testChatCommand_validation() {
    ChatCommand cmd = new ChatCommand();
    fakeClientHandler.setFakeUser(bidder);

    // Empty message
    cmd.execute(fakeClientHandler, PacketReq.of(RequestType.CHAT, new ChatRequest("   ")));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    // Null message
    cmd.execute(fakeClientHandler, PacketReq.of(RequestType.CHAT, new ChatRequest(null)));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    // Too long message
    cmd.execute(
        fakeClientHandler, PacketReq.of(RequestType.CHAT, new ChatRequest("a".repeat(501))));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    // Coverage
    assertEquals(ResponseType.CHAT_RESULT, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testDisableAutoBidCommand_validation() {
    DisableAutoBidCommand cmd = new DisableAutoBidCommand(autoBidService);
    fakeClientHandler.setFakeUser(bidder);

    cmd.execute(
        fakeClientHandler,
        PacketReq.of(RequestType.DISABLE_AUTO_BID, new DisableAutoBidRequest(0)));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    assertEquals(ResponseType.DISABLE_AUTO_BID_RESULT, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testCreateAuctionCommand_validation() {
    CreateAuctionCommand cmd = new CreateAuctionCommand(auctionService, queryService);
    fakeClientHandler.setFakeUser(seller);

    // Negative price
    CreateAuctionRequest badPayload =
        new CreateAuctionRequest(
            "", "", -100L, 100L, ItemType.ART, 60, LocalDateTime.now().plusHours(1));
    cmd.execute(fakeClientHandler, PacketReq.of(RequestType.CREATE_AUCTION, badPayload));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    assertEquals(ResponseType.CREATE_AUCTION_RESULT, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testUpdateAuctionCommand_validation() {
    UpdateAuctionCommand cmd = new UpdateAuctionCommand(auctionService, queryService);
    fakeClientHandler.setFakeUser(seller);

    UpdateAuctionRequest badPayload =
        new UpdateAuctionRequest(
            -1, "", "", 1000L, 100L, ItemType.ART, 60, LocalDateTime.now().plusHours(1), -1);
    cmd.execute(fakeClientHandler, PacketReq.of(RequestType.UPDATE_AUCTION, badPayload));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    assertEquals(ResponseType.UPDATE_AUCTION_RESULT, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testPlaceBidCommand_validation() {
    PlaceBidCommand cmd = new PlaceBidCommand(bidService, userService, queryService);
    fakeClientHandler.setFakeUser(bidder);

    cmd.execute(
        fakeClientHandler, PacketReq.of(RequestType.PLACE_BID, new PlaceBidRequest(0, 100L)));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    assertEquals(ResponseType.PLACE_BID_RESULT, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testSetAutoBidCommand_validation() {
    SetAutoBidCommand cmd = new SetAutoBidCommand(autoBidService, queryService);
    fakeClientHandler.setFakeUser(bidder);

    cmd.execute(
        fakeClientHandler,
        PacketReq.of(RequestType.SET_AUTO_BID, new SetAutoBidRequest(0, 1000L, 100L)));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    assertEquals(ResponseType.SET_AUTO_BID_RESULT, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testFetchAuctionResultCommand_validation() {
    FetchAuctionResultCommand cmd = new FetchAuctionResultCommand(auctionService, userService);
    fakeClientHandler.setFakeUser(bidder);

    cmd.execute(
        fakeClientHandler,
        PacketReq.of(RequestType.FETCH_AUCTION_DETAIL, new AuctionResultRequest(0)));
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    assertEquals(ResponseType.AUCTION_RESULT_FETCHED, cmd.responseType());
    assertNotNull(cmd.unexpectedErrorMessage());
  }

  @Test
  public void testUploadAvatarCommand_exceptions() {
    UploadAvatarCommand cmd = new UploadAvatarCommand(userService, imageStorageService);
    fakeClientHandler.setFakeUser(bidder);

    // IllegalArgumentException (invalid base64)
    PacketReq reqBadBase64 =
        PacketReq.of(
            RequestType.UPLOAD_AVATAR, new UploadAvatarRequest("invalid_base64!", "avatar.png"));
    cmd.execute(fakeClientHandler, reqBadBase64);
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());

    // ServiceException (invalid user ID)
    User fakeUser =
        new User(99999, "fake", new Account("fake", "fake", UserRole.BIDDER), new Wallet());
    fakeClientHandler.setFakeUser(fakeUser);
    String base64 = java.util.Base64.getEncoder().encodeToString("avatar_data".getBytes());
    PacketReq reqFakeUser =
        PacketReq.of(RequestType.UPLOAD_AVATAR, new UploadAvatarRequest(base64, "avatar.png"));
    cmd.execute(fakeClientHandler, reqFakeUser);
    assertFalse(fakeClientHandler.getSentPacket().isSuccess());
  }
  ...
  public static class FakeClientHandler extends ClientHandler {
    ...
    private final java.util.List<PacketRes> sentPackets = new java.util.ArrayList<>();
    ...
    public PacketRes getSentPacket() {
      return sentPackets.isEmpty() ? null : sentPackets.get(sentPackets.size() - 1);
    }

    public PacketRes getSentPacket(ResponseType type) {
      for (PacketRes p : sentPackets) {
        if (p.getType() == type) {
          return p;
        }
      }
